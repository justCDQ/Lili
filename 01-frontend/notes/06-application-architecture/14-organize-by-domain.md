...
领域模块以一组共同变化的业务规则为边界，并通过公开入口提供用例、事件和稳定类型。模块内部仍可按 UI、application、domain 与 infrastructure 分层，但这些技术子层不能成为全仓库任意导入的全局目录。
...
下面代码通过领域公开入口暴露用例和类型，模块内部保留 UI、规则与适配器。消费者只能导入入口，目录移动测试用来确认外部代码没有依赖内部文件。
...
模块类型能描述公开 API，却不能阻止深层路径导入或共享目录反向依赖业务。需要用 exports、lint 边界和构建图执行规则，并在运行测试中验证适配器输入。
...
领域边界应依据业务语言、共同变化和团队所有权，而不是页面相邻或文件类型。只有真正跨领域稳定的能力才进入 shared，并为其定义更严格的兼容责任。
...
先从一次需求变更列出同时修改的文件，再检查它们是否跨越公开入口或通过 shared 偷渡领域规则，最后查看循环和发布影响。失败信号是小需求横跨多个技术目录、深层导入泛滥或领域测试必须启动整站；用变更耦合、依赖图和模块级测试验证边界。
...
集成时领域之间通过公开用例、事件或稳定 DTO 协作，不读取对方内部 store 和组件。迁移期间保留兼容入口并统计旧路径使用量，清零后再删除。

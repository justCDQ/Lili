...
全局 store 是应用实例内的客户端事实容器，适用于无法由共同父组件自然拥有且需要跨子树持续存在的状态。读取应通过窄选择器，写入应通过表达业务意图的命令，实例必须在 SSR 请求、测试和微前端边界上隔离。
...
下面代码把 store 创建、命令和选择器分离，并允许每个应用实例独立销毁。React 绑定只负责订阅，网络缓存和持久化由各自边界接入。
...
动作和状态类型不能阻止跨请求复用单例，也不能检测宽选择器造成的无关渲染。测试应为每例创建 store，验证销毁后无订阅，并用渲染计数检查选择器范围。
...
只有身份、主题或跨远距离流程等真正拥有应用级生命周期的客户端事实才进入全局 store。远端实体交给查询缓存，局部表单与交互状态留在组件；store 库不会替团队划清这些边界。
...
先确认异常值由哪个命令写入，再检查选择器是否订阅了过宽对象，最后排查 store 实例是否跨测试、SSR 请求或微前端共享。失败信号包括无关组件连锁渲染、测试相互污染和用户状态串线；用命令日志、订阅计数和实例隔离测试验证。
...
集成时全局 store 只保存自身权威的应用状态，通过命令暴露写入口，通过窄选择器暴露读取。查询结果、URL 参数和表单草稿只保存引用或派生视图，不复制成第二份可写实体。

...
所有者必须拥有该事实的写入命令、冲突决策和版本推进；其他位置只能保存可重建投影、有限期缓存或明确草稿。两个位置若都能独立接受写入并靠 Effect 对齐，就已经形成双主而不是单一数据源。
...
下面代码用显式命令维护一个权威状态，并从它计算只读投影。外部缓存和 DOM 只通过适配器报告事件，不能绕过所有者直接改写投影。
...
只读类型可以限制调用方式，却不能发现同一事实被复制到 URL、store 和组件后分别更新。应记录每次写入的所有者和版本，用竞态测试验证迟到事件不会覆盖新事实。
...
决定所有者时要问谁能权威确认、谁负责冲突以及事实活多久，而不是谁最方便调用 `setState`。缓存、草稿和投影可以并存，但必须是单向可重建或有明确合并协议。
...
先为异常字段列出所有可写位置和写入时间线，再确认哪一个才是权威所有者，最后检查派生值是否被 Effect 反向同步。失败信号是两个 setter 修改同一事实、返回导航后状态分叉或缓存显示假成功；用写入日志、延迟响应和重放时间线验证。
...
集成时为 URL、表单、查询缓存和领域实体分别标注权威范围，只跨边界传命令、版本和只读投影。需要冲突时显式进入冲突状态，不用双向 Effect 维持表面一致。

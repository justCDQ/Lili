...
客户端保存的是带查询参数、获取时间和状态的远端快照，不是实体的最终所有权。查询键决定快照身份，`staleTime` 决定何时允许重新验证，失效只表示需要重取而不等于立刻删除数据。
...
下面代码展示查询键、取消信号和失效策略的契约，真实请求由查询函数边界执行。测试需控制时钟与响应顺序，验证旧响应不会覆盖新参数对应的数据。
...
响应类型不会证明服务端真的返回该结构，也不会处理竞态；查询函数仍要做运行时校验并消费取消信号。用反序响应、离线和 401 夹具验证缓存不会把错误事实传播给界面。
...
共享远端快照、失效与并发去重适合查询缓存，未提交表单和纯交互状态不应塞入其中。缓存库提供机制，但查询键、新鲜度和乐观冲突策略必须由业务语义决定。
...
先打印规范查询键和缓存条目，再检查请求是否携带取消信号、响应版本是否仍匹配当前参数，最后核对 mutation 后失效范围。失败信号是不同用户命中同一键、旧响应回写或乐观值永久残留；用隔离 QueryClient、反序响应和失败回滚测试验证。
...
集成时远端实体只由查询缓存保存，组件通过选择器读取投影，表单只保存正在编辑的草稿。提交成功后用服务端结果或精确失效更新缓存，不再同步写一份全局实体副本。

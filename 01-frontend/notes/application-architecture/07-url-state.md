...
下面代码集中实现 URL 的解析、规范化与序列化，组件只消费已验证的路由状态。浏览器历史写入留在路由边界，以便单测往返不变量和非法参数回退。
...
URL 是不可信外部输入，TypeScript 不能保证枚举值、页码范围或编码合法。运行时必须解析并规范化，再用往返测试确认 `parse(serialize(state))` 保持同一语义。
...
需要分享、刷新恢复或前进后退的状态适合进入 URL；秘密、临时输入和高频拖拽值不适合。路由库只负责读写历史，仍需由产品定义参数的默认值、兼容期和规范地址。
...
先保存原始地址并检查解析结果，再确认规范化是否丢失参数，最后观察 `popstate` 后 UI 是否从当前 location 重新派生。失败信号是复制链接不能复现、后退后界面不变或秘密进入日志；用往返属性测试、历史导航测试和日志扫描验证。
...
集成时 URL 负责可导航筛选条件，查询缓存用规范参数组成键，表单草稿仍由表单拥有。组件不能把 URL 初始化值复制为长期可写 state，否则历史导航会出现两个事实源。

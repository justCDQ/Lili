...
一次请求依次经过参数规范化、发送、响应校验、错误归类、缓存写入和可选重试。缓存键回答“这是哪个远端快照”，错误联合回答“为何失败”，重试策略只根据幂等性与错误类别决定下一次尝试。
...
下面代码把响应校验、错误联合、缓存键和重试策略组合为请求管线。实际 fetch 与时钟由适配器注入，测试可精确推进退避并复现取消。
...
声明响应泛型不会校验服务器数据，必须在边界解析 Schema 后才能进入缓存。错误联合还要保留可重试性、状态码和 trace ID，让 UI 与监控使用同一分类。
...
只有幂等且被判定为瞬时故障的请求才能自动重试；认证、校验和业务拒绝应直接返回。缓存新鲜度与重试上限按业务成本定义，不能照搬库默认值。
...
先沿 trace ID 核对一次请求的发送、响应校验和错误分类，再检查缓存键是否命中错误实体，最后重放退避与取消时间线。失败信号是 4xx 无限重试、不同参数共享缓存或响应字段让渲染崩溃；用假时钟、Schema 失败夹具和请求计数验证。
...
集成时请求层输出稳定错误联合，缓存层管理远端快照，UI 只根据错误类别决定重试或提示。各层共享 trace 上下文但不互相复制状态机。

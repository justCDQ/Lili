...
| `autocorrect` | 控制可编辑文本是否启用浏览器/系统自动纠错 | 人名字段关闭纠错、正文编辑器开启纠错 | 是枚举属性且不是 Baseline；`email`、`url`、`password` 等输入类型不启用；纠错实现取决于浏览器和操作系统 |
| `writingsuggestions` | 控制浏览器提供的续写建议 | 网站自带建议系统时关闭浏览器建议 | 使用 `true` / `false`；支持范围有限，必须做特性检测并保证无该能力时正常降级 |

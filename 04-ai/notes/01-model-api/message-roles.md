...
先读 [HTTP、JSON、REST API](../00-foundations/http-json-api.md) 与 [多轮等核心能力](core-api-capabilities.md)。本文不讨论 Tool 输出项的完整协议，只说明它不能伪装成普通角色消息。
...
若模型仍遵循旧 `assistant` 消息，应用显示“无法完成操作”，记录 02-prompt/模型版本并加入回归集。即使模型生成取消工具调用，订单服务也因状态不允许而返回确定性业务错误；模型角色无法绕过它。

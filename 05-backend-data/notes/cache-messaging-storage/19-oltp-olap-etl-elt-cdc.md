...
CDC 必须保留源事务顺序、日志位置、主键、操作类型和行版本，才能把 OLTP 变更可靠地投影到 OLAP。快照与增量的交界、更新/删除语义和 schema 演进都需要可验证；只看到目标表“有数据”不能证明没有缺口或重复。
...
CDC watermark 应明确它表示已读取、已写入还是已对账的源日志位置，并按源分片持久化。只有该位置之前的更新与删除都已幂等落入目标，才能用它计算复制延迟或作为安全回填边界。
...
从已确认的源 LSN 与消费 offset 恢复，重复变更按订单主键和源版本幂等合并。若 ClickHouse 写入已成功但 offset 未提交，允许重放而不能重复计量；快照重建写入新表并持续追平 WAL，按 count、金额、删除和最大版本对账后再切换查询。
...
同一输入 manifest/checksum 生成稳定 batch ID，解析结果写该批次的隔离分区。失败重跑先清理或覆盖未发布分区，不触碰已审批报表；坏行修正规则产生新 run 与质量报告，总额和行数重新守恒后才原子替换该业务日版本。
...
1. 对每个源表记录快照起止 LSN、流起点和当前确认 LSN，证明 snapshot 与 stream 无 gap、无重叠漏算。
2. 按 tenant/day 比较源端与目标端存活主键数；append-only 事件表另比较事件 ID 去重前后数量。
3. 金额和数量使用源端可复算的整数或 decimal 聚合对账，任何非零财务差异阻断发布。
4. 必填字段按 schema 版本检查；新增列在回填完成前允许的 null 范围必须显式限定到旧版本数据。
5. 更新表比较每个分片的最大源版本与目标版本，并抽查同一主键的最终字段 hash。
6. 删除单独统计源 delete、传输 tombstone、目标失效行；删除延迟超过合规窗口立即告警。
7. CDC freshness 同时报“当前时间减源 commit time”和“最新已处理 LSN”，避免无新写入时误判。
8. schema 变更先验证主键、类型兼容和默认值；无法解析的事件进入隔离 topic，不能跳过并推进确认位点。
9. 回填与实时流按主键版本仲裁，检查旧回填不会覆盖更高版本的在线变更。
10. 每日固定抽样主键，从 OLTP 当前行、变更日志到 OLAP 最终行逐字段核验并保存差异证据。
...
CDC 排障选择一个订单主键和时间区间，依次核对源行版本、WAL/LSN、Debezium envelope、Kafka partition/offset、目标 staging 版本与最终查询行；批 ETL 则从对象 checksum、rejected 清单、发布分区追到报表 SQL。连接器 running 或 DAG success 都不能替代端到端对账。

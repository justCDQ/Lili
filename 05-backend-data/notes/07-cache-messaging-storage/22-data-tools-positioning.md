...
这些工具解决的是不同责任：Kafka 保存可回放事件，Flink 维护有状态流计算，Spark 执行大规模批处理，Airflow 编排有界任务，dbt 管理 SQL 模型与测试，ClickHouse 服务分析查询。可靠链路需要明确每段输入/输出契约和唯一权威状态，不能靠堆叠工具获得端到端一致性。
...
Flink 从已完成 checkpoint 恢复 Kafka offsets 和算子状态，ClickHouse sink 以事件/窗口版本幂等。checkpoint 不可用时从对象湖选定时间范围重放到隔离表，Airflow 编排批对账；实时表与日批在固定水位比较通过后，才发布校正版本，避免两个写入者互相覆盖。
...
Airflow 以业务月和输入 manifest 生成稳定关账 run，Spark/dbt 重跑写候选 schema。任何测试或审批失败都保留上一 close version；修复 run 重新执行受影响依赖并产生新的 lineage/测试证据，财务 owner 批准后才更新 current-close 指针，已批准版本保持不可变。
...
1. Kafka topic 记录生产/消费 schema、partition key 和 retention，验证重放窗口覆盖最大可恢复时长。
2. Flink checkpoint 检查完成率、持续时间、对齐/非对齐模式和状态大小；失败不得推进外部发布水位。
3. 流 sink 的 exactly-once 或幂等条件用故障注入验证，不能从框架配置名称推断端到端语义。
4. Spark 输入输出按分区统计行数、业务键和金额，shuffle 倾斜导致的 task retry 不得制造重复文件。
5. Airflow DAG 的 data interval、logical date 和重跑参数一致；补数不能误读“当前日期”。
6. dbt source freshness、唯一性、引用和业务测试在候选 schema 运行，失败阻断下游发布任务。
7. ClickHouse 检查写入版本、后台 merge backlog、分区数和查询扫描量，表引擎语义需匹配更新模型。
8. 每个跨工具交接保存 topic offset、object manifest、table snapshot 或 close version，形成可复现边界。
9. 实时结果与批校正按相同 metric version 和截止水位对账，不比较仍在变化的开放窗口。
10. 端到端演练 Kafka 重放、Flink 恢复、Spark 补数、dbt 失败和 ClickHouse 回滚，记录每步恢复时限。
...
按失败边界选择证据：Kafka 查 partition/offset，Flink 查 checkpoint 与 backpressure，Spark 查 stage/shuffle 和输出 manifest，Airflow 查 data interval 与 task instance，dbt 查 compiled SQL/test，ClickHouse 查 part/merge/query log。最后用同一事件 ID 或关账业务键串起各工具，避免把下游错误归因给最显眼的组件。

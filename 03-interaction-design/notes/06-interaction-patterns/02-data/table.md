...
默认列由最高频比较任务决定，不由后端字段顺序决定。对象标识、当前状态和主比较值优先；低频详情、内部 ID 和只在异常时需要的字段进入详情或列配置器。默认列必须在常见桌面宽度下保留主任务，不以横向滚动隐藏关键状态。

列定义包含稳定 `columnId`、标题、数据类型、单位、格式化、排序能力、筛选能力、权限要求、默认宽度、最小/最大宽度和固定资格。展示名称可以翻译，保存配置不能使用翻译后标题作主键。

### 列配置与保存视图

列配置支持显示/隐藏、顺序、宽度和允许固定的列。至少提供键盘可操作的移动方式，拖拽不是唯一入口。恢复默认值只重置列设置，不应清空用户筛选或业务数据。
...
{
  "viewVersion": 3,
  "columns": ["invoiceNo", "supplier", "amount", "status"],
...
重新打开视图时，服务端或受控配置层删除已失权、已弃用或不再支持的字段，并向用户说明视图已调整。公开共享视图不能携带创建者的权限；每个查看者按自身数据范围重新查询。

### 固定列与横向滚动

固定列只保留识别当前行和执行核心任务所需内容。固定过多会压缩可滚动区域。固定层、滚动层和表头必须来自同一逻辑表格，行高和焦点不能形成两套对象。

横向滚动容器具有可访问名称，键盘可到达，并在边缘提供非颜色的溢出提示。窄屏应评估是否改为摘要列表加详情，而不是无限压缩列。

### 不可选行

不可选行保留行内容并说明原因，例如“已结算，不能再次付款”或“你没有付款权限”。表头全选只作用于当前合法范围，摘要同时显示可选与不可选数量。

服务端仍逐项授权；前端禁用复选框不能阻止构造 ID。若原因对用户不可公开，使用最小必要说明并提供合法处理路径。

### 行内编辑、树形数据与保存

行内编辑需要明确浏览模式和编辑模式、原值、草稿值、校验、保存中、成功、失败与冲突。批量编辑不能把一个字段的错误覆盖整行；保存结果按稳定行 ID 和字段 ID 返回。

树形表格同时表达父子层级与列关系。展开状态、选择范围和聚合值要定义：选择父节点是否包含未加载子节点、移动节点是否改变权限、折叠后活动焦点如何恢复。无法完整实现 treegrid 键盘模型时，使用树导航进入普通子表格。

### 权限字段与大数据

字段权限必须在查询、排序、筛选、列配置、导出和聚合中保持一致。隐藏列不能仍出现在 URL 建议或排序白名单；脱敏值不能作为导出原值。

大数据先选择服务端筛选、稳定排序和游标分页。只有真实测量证明分页不足时才引入虚拟化；虚拟化需要维护逻辑行列总数、稳定焦点、动态行高和辅助技术可用性。

### 专项验证

- 保存视图后撤销一个字段权限，重新打开不泄露字段且提示调整；
- 列标题切换语言后仍按稳定 ID 恢复配置；
- 固定列、200% 缩放和窄屏下焦点始终可见；
- 全选 500 项时明确排除 17 个不可选对象；
- 编辑期间服务端更新同行，保存进入字段级冲突而非静默覆盖；
- 树节点折叠、移动和懒加载后选择数量与服务端一致；
- 200 万行查询的 UI、导出与权限范围可以对账。

## 来源

- [WHATWG HTML：Tables](https://html.spec.whatwg.org/multipage/tables.html)（访问日期：2026-07-22）
- [W3C APG：Grid Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/grid/)（访问日期：2026-07-22）

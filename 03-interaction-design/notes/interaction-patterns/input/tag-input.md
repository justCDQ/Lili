...
Tag Input 标签输入管理一个可增删的值集合，并协调文本编辑、候选选择、去重与令牌级删除。它适合标签数量有限且每项身份清楚的任务，不适合用来编辑任意长列表。
...
每个令牌要区分稳定 ID、显示标签、来源和有效状态，输入框只保存尚未提交的查询。去重、数量上限和新建权限基于规范化身份判断，不能仅比较屏幕文字。
...
保留正在输入的查询和已生成令牌的原始标签。逗号、Enter 或失焦是否完成一个标签必须显式定义，输入法组合期间的逗号和 Enter 不能提前创建令牌。

### 规范化值

已有标签按标签 ID 去重；允许自由创建时，再按产品声明的大小写、Unicode 和空白规则生成规范键。`C++`、`c++` 是否同一标签属于业务规则，不能由组件擅自决定。

### 草稿

草稿保存有序令牌 ID、临时新标签和未完成查询。恢复时批量验证标签是否仍存在、是否被合并以及用户是否仍可使用；被合并标签应迁移到权威 ID 并告知用户。

### 权威结果

提交后服务端返回最终标签 ID 集合和对象版本。新建标签可能被服务端合并到已有标签，界面必须采用返回 ID 和标签，而不是保留客户端临时令牌。
...
验收要覆盖自由创建开关、最大数量、同名标签、合并标签、Backspace 删除和输入法组合，证明集合身份和键盘操作不会相互混淆。
...
| 令牌被拒绝 | 标签已停用、数量超限或无新建权限 | 标记具体令牌，保留其他有效令牌和查询 | 移除、替换或申请权限 |
| 冲突 | 基础对象版本变化 | 比较、刷新或合并 | 新版本确认 |
| 集合结果未知 | 保存请求超时 | 冻结再次提交，读取对象当前标签集合与版本 | 对账后继续编辑或完成 |
...
服务端把临时标签 `frontend-new` 解析为既有标签 `tag-81` 后，组件替换临时 ID、更新显示名并保留原顺序；重复标签只出现一次。
...
窄屏令牌在输入框前自然换行，删除按钮不覆盖标签文本；令牌过多时提供计数和可滚动区域，但 Tab 顺序仍按 DOM 集合顺序。离线新建令牌明确标为未验证。
...
认证恢复后批量校验所有令牌：有效标签继续保留，停用或受限标签逐项标错，未完成查询保持文本状态；不能因为一个标签失败而清空整个集合。
...
在 320 CSS px 等效宽度和 200% 缩放下，令牌允许多行换行，文本和删除按钮都保持可辨认；输入框至少占一整行，错误说明紧跟对应令牌或集合。
